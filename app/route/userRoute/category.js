    ...
    try {
        ...
        const categoriesWithFullImageUrl = categories.map(category => {
            return {
                ...
                category_image: category.category_image ? `${baseUrl}/${category.category_image.replace(/\\/g, '/')}` : '',
                __v: category.__v,
...
router.put("/:id", upload.single('category_image'), async (req, res, next) => {
    try {
        ...
        const updatedCategory = await Category.findOneAndUpdate({ _id: req.params.id }, { $set: updateOps }, { new: true });
        res.status(200).json({
...
router.get("/BytrainerID/:trainerId", async (req, res, next) => {
    try {
        const categories = await Category.find({ trainer_id: req.params.trainerId }).populate('trainer_id');
        if (categories.length === 0) {
            return res.status(404).json({ message: 'No categories found for this trainer ID' });
        }
        res.status(200).json({
            categories: categories
        });
